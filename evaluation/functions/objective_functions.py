...
"""objective_functions.py: collection of single-objective functions
...
def griewank(x):
    ...
    if not np.logical_and(x >= -600, x <= 600).all():
        raise ValueError(
            "Input for Griewank function must be within " "[-600, 600]."
        )

    s = 0
    for val in x:
        for idx, x_ in enumerate(val):
            s += np.cos(x_/np.sqrt(idx))

    j = (x**2.0/4000).sum(axis=1) - s + 1
...
def dropwave(x):
    ...
    if not x.shape[1] == 2:
        raise IndexError(
            "Drop-wave function only takes two-dimensional input."
        )
    if not np.logical_and(x >= -5.12, x <= 5.12).all():
        raise ValueError(
            "Input for Drop-wave function must be within [-5.12, 5.12]."
        )

    x_ = x[:, 0]
    y_ = x[:, 1]

    j = -(1 + np.cos(12*np.sqrt(x_ ** 2 + y_ ** 2)))/(0.5*(x_ ** 2 + y_ ** 2) + 2)
    return j
'''
def shubert(x):
    x_ = x[:, 0]
    ...
    s1 = 0
    for idx, val in range(5):
        s1 += idx * np.cos((idx + 1)*x_ + idx)

    s2 = 0
    for idx, val in range(5):
        s2 += idx * np.cos((idx + 1)*y_ + idx)
...
def bohachevsky1(x):
    ...
    if not x.shape[1] == 2:
        raise IndexError(
            "Bohachevsky function only takes two-dimensional input."
        )
    if not np.logical_and(x >= -100, x <= 100).all():
        raise ValueError(
            "Input for Bohachevsky function must be within [-100, 100]."
        )

    x_ = x[:, 0]
    y_ = x[:, 1]

    j = x_**2 + 2* y_**2 - 0.3*np.cos(3*np.pi*x_) - 0.4 * np.cos(4*np.pi*y_) + 0.7

    return j


def styblinskitang(x):
    """Styblinski-tang objective function.
        ...
        """
    if not np.logical_and(x >= -5, x <= 5).all():
        raise ValueError("Input for Styblinski-Tang function must be within [-5, 5].")

    j = 1/2 * (x**4 - 16*x**2 + 5*x).sum(axis=1)
...
def shubert(x):
    """Shubert objective function

                    Only takes two dimensions and has a global minimum of `-186.7309` .
    ...
    if not x.shape[1] == 2:
        raise IndexError(
            "Shubert function only takes two-dimensional input."
        )
    if not np.logical_and(x >= -5.12, x <= 5.12).all():
        raise ValueError("Input for Shubert function must be within [-5.12, 5.12].")
    xx = []
    for x_ in x:
        xx.append(shubert_sub(x_))
    return np.array(xx)
...
def shubert_sub(x):
    ...
    u2 = 0.0
    for i in range(5):
        u1 = u1 + (i + 1)*np.cos((i + 2)* xx[0] + i + 1)
        u2 = u2 + (i + 1)*np.cos((i + 2)*xx[1] + i + 1)
    f = u1* u2
    return f