...
class EFOAgent(Optimizer):
    ...
    def __init__(self, id, problem, window_size=10000, pop_size=100, r_rate=0.3, ps_rate=0.85, p_field=0.1, n_field=0.45, **kwargs):
        """
        Args:
            problem (dict): The problem dictionary
            window_size (int): maximum number of iterations, default = 10000
        ...
        self.epoch = self.validator.check_int("epoch", window_size, [1, 100000])
        self.pop_size = self.validator.check_int("pop_size", pop_size, [10, 10000])
        self.r_rate = self.validator.check_float("r_rate", float(r_rate), (0, 1.0))
        self.ps_rate = self.validator.check_float("ps_rate", float(ps_rate), (0, 1.0))
        self.p_field = self.validator.check_float("p_field", float(p_field), (0, 1.0))
        self.n_field = self.validator.check_float("n_field", float(n_field), (0, 1.0))
...
class OriginalEFO(EFOAgent):
    ...
    def __init__(self, problem, window_size=10000, pop_size=100, r_rate=0.3, ps_rate=0.85, p_field=0.1, n_field=0.45, **kwargs):
        """
        Args:
            problem (dict): The problem dictionary
            window_size (int): maximum number of iterations, default = 10000
            ...
            n_field (float): default = 0.45    portion of population, negative field
        """
        super().__init__(problem, window_size, pop_size, r_rate, ps_rate, p_field, n_field, **kwargs)
        self.nfe_per_epoch = self.pop_size
        self.sort_flag = True
