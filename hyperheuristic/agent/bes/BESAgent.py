...
class BESAgent(Optimizer):
    ...
    def __init__(self, id, problem, window_size=10000, pop_size=100, a_factor=10, R_factor=1.5, alpha=2.0, c1=2.0, c2=2.0, **kwargs):
        """
        Args:
        ...
        self.a_factor = self.validator.check_int("a_factor", int(a_factor), [2, 20])
        self.R_factor = self.validator.check_float("R_factor", float(R_factor), [0.1, 3.0])
        self.alpha = self.validator.check_float("alpha", float(alpha), [0.5, 3.0])
        self.c1 = self.validator.check_float("c1", float(c1), (0, 4.0))
        self.c2 = self.validator.check_float("c2", float(c2), (0, 4.0))
