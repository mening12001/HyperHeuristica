...
class AOAAgent(Optimizer):
    ...
    def __init__(self, id, overall_nr_iterations, problem, window_size=10000, pop_size=100, alpha=5, miu=0.5, moa_min=0.2, moa_max=0.9, **kwargs):
        """
        Args:
        ...
        self.alpha = self.validator.check_int("alpha", int(alpha), [2, 10])
        self.miu = self.validator.check_float("miu", float(miu), [0.1, 2.0])
        self.moa_min = self.validator.check_float("moa_min", float(moa_min), (0, 0.41))
        self.moa_max = self.validator.check_float("moa_max", float(moa_max), (0.41, 1.0))
    ...
    def evolve(self, epoch):
        ...
        """
        moa = self.moa_min + (self.overall_nr_iterations  + epoch+1) * ((self.moa_max - self.moa_min) /200)  # Eq. 2
        mop = 1 - ((self.overall_nr_iterations + epoch+1) ** (1.0 / self.alpha)) / (200** (1.0 / self.alpha))  # Eq. 4
