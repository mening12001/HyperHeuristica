...
from metaheuristic.optimizer.problem import Problem
...
class Optimizer:
    ...
    def __init__(self, id, problem, kwargs=None):
        """
        Args:
    ...
    def get_target_wrapper(self, position):
        ...
        objs = self.problem.fit_func(self.element_to_list(position, len(position)))
        if not self.problem.obj_is_list:
            objs = [objs]
        fit = np.dot(objs, self.problem.obj_weights)
        return [fit, objs]

    def element_to_list(self, X, dim):
        x_temp_list = np.zeros((1, dim))
        x_temp_list[0] = X
        return x_temp_list

    def create_solution(self, idx, lb=None, ub=None):
        """
        ...
        if self.initial_positions is not None:
            position = self.initial_positions[idx]
        else:
    ...
    def solve(self, mode='sequential'):
        ...
        for epoch in range(0, self.epoch):
            ...
            solutions_per_iteration[epoch] = self.to_actual_solutions(self.pop, self.id)
        ...
        return solutions_per_iteration

    def evolve(self, epoch):
        pass

    def to_actual_solutions(self, population, id):
        costs_touples = []
        for idx, sol in enumerate(population):
            costs_touples.append(
                ActualSolution(id = id, value = sol[self.ID_TAR][self.ID_FIT], position=sol[self.ID_POS], velocity=None, pbest_pos=None,
                               pbest_cost=None))
        return costs_touples

    def init_initial_positions(self, actual_solutions):
        if actual_solutions is not None:
            self.initial_positions = [solution.position for solution in actual_solutions]
    ...
    def create_population(self, pop_size=None):
        ...
        Returns:
            list: population or list of solutions/agent
        """
        if pop_size is None:
        ...
        '''
        pop = [self.create_solution(idx, self.problem.lb, self.problem.ub) for idx in range(0, pop_size)]
        return pop
