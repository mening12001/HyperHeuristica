...
from pyswarms.backend import rep
...
def compute_velocity(swarm, clamp, vh, bounds=None, global_solutions=None):
    ...
    try:
        ...
        ensemble = np.zeros(swarm_size)
        ct = 0
        for global_best in global_solutions:
            if not np.array_equal(global_best, swarm.best_pos):
                ct += 1
                #ensemble += c3 * np.random.uniform(0, 1, swarm_size) * (global_best - swarm.position)
        for global_best in global_solutions:
            if not np.array_equal(global_best, swarm.best_pos):
                #ensemble += 2/ct * np.random.uniform(0, 1, swarm_size) *  np.sign(swarm.position - global_best) * (1 - abs(swarm.position - global_best)/(U - L))
                ensemble += 2/ct * np.random.uniform(0, 1, swarm_size) * (global_best - swarm.position)
                #ensemble -=  2/9 *np.random.uniform(0, 1, swarm_size) * (global_best - swarm.position)
        ...
        temp_velocity = (w * swarm.velocity) + cognitive + social# + ensemble
        updated_velocity = vh(
...
def compute_position(swarm, bounds, bh):
    ...
    average_velocity = numpy.average(swarm.velocity, axis=0)

    diversity_control = (
            c3
            * np.random.uniform(0, 1, swarm_size)
            * (swarm.position - average_velocity)
    )

    #temp_position = swarm.position.copy()
    temp_position += swarm.velocity
    temp_position +=diversity_control
